...
class Order extends Model
{
   ...
   public function scopeJoinUser($query)
    {
        $query = $query->select('payments.*');
        $query = $query->leftJoin('users', 'payments.id', '=', 'users.id');
  //      $query = $query->leftJoin('address', 'payments.user_id', '=', 'users.id');
...
/*   public function scopeMostRecentEvents($query)
    {
        return $query->select('user_id', 'event_type', Capsule::raw('MAX(time) as occurred_at'))
            ->groupBy('user_id')
            ->groupBy('type');
    }
  ...
  /*  public function scopeMostRecentEventsByType(Builder $query, $type)
      ...
      public function user()
    {
       ...
       $classMapper = static::$ci->classMapper;
        ...
        return $this->belongsTo($classMapper->getClassMapping('user'),'id');
     }
