...
public function getCurrent(Request $request, Response $response, $args) {
        return $response->withJson($this->ci->currentUser,200,JSON_PRETTY_PRINT);
}
    ...
    public function getTransactions (Request $request, Response $response, $args)
    {
        ...
        $sprunje = $classMapper->createInstance('transaction_sprunje', $classMapper, $params);
    ...
    public function getModalConfirmDelete(Request $request, Response $response, $args)
    {
        ...
        return $this->ci->view->render($response, 'modals/confirm-delete-user.html.twig', [
            'user' => $user,
            'form' => [
                'action' => "userWallet/api/users/u/{$user->user_name}",
            ]
        ]);
    }
